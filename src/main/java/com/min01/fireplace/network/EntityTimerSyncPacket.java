	...
	public static class Handler 
	{
		public static boolean onMessage(EntityTimerSyncPacket message, Supplier<NetworkEvent.Context> ctx) 
		{
			ctx.get().enqueueWork(() ->
			{
				if(!message.reset)
				{
					if(FireplaceUtil.CLIENT_TIMER_MAP.containsKey(message.uuid))
					{
						FireplaceUtil.CLIENT_TIMER_MAP.remove(message.uuid);
					}
					FireplaceUtil.CLIENT_TIMER_MAP.put(message.uuid, new EntityTimer(message.tickRate, 0));
				}
				else
				{
					if(FireplaceUtil.CLIENT_TIMER_MAP.containsKey(message.uuid))
